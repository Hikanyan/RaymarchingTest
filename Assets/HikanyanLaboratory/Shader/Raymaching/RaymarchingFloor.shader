Shader "Unlit/RaymarchingFloor"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float map(float3 p)
            {
                // 中心が(0, 1, 0)で半径が1の球体の距離関数
                float sphere = sdSphere(p - float3(0, 1, 0), 1);

                // 法線が(0, 1, 0)で原点からの距離が0の平面の距離関数
                float plane = sdPlane(p, float3(0, 1, 0), 0);

                // 2つの距離関数を最小値が和集合になります
           ...
           float4 frag(v2f input): SV_Target
            {
                ...
                for (i = 0; i < 500; i++)// ループ回数を99から500に変更
                {
